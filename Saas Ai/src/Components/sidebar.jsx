...
const Sidebar = ({ sidebar, setSidebar }) => {
  ...
  // FIX: guard against undefined user (prevents runtime crash)
  if (!user) return null;
  ...
  return (
    <div
      className={`w-60 bg-white border-r border-gray-200 flex
        ...
        bottom-0 ${sidebar ? 'translate-x-0' : 'max-sm:-translate-x-full'} transition-all
    ...
    >
      ...
      <div className='w-full border-t border-gray-200 p-4 px-7 flex items-center justify-between'>
        <div onClick={openUserProfile} className='flex gap-2 items-center cursor-pointer'>
          <img src={user.imageUrl} className='w-8 rounded-full' alt='User profile'/>
        </div>
        <div>
